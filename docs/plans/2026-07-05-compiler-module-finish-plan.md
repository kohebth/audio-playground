...
Compiler receives validated contracts plus metadata, expands projects/units, resolves graph bindings, and emits all graph facts needed by runtime-image.
...
- Runtime-image no longer infers bypass endpoints from node IDs.
...
- [ ] Add tests proving runtime-image still builds after `compiled_node.atom` is nulled on a normal compiled plan.

## Tests

- Extend `test_unit_v2_compile` for compiled atom layout names, thunks, sizes, and field counts.
- Extend `test_runtime_image_v2` for runtime-image build after nulling compiled node registry pointers.
