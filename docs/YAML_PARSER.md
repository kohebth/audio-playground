...
int main(int argc, char **argv) {
    ...
    char *src = uc_arena_alloc(&arena, (size_t)size + 1u, 1u);
    ...
    uc_node *root = NULL;
    uc_error err = {0};
    if (uc_lex(src, (size_t)size, &arena, &toks, &err) != UC_OK ||
        uc_parse(&toks, &arena, &root, &err) != UC_OK) {
    ...
    const uc_node *name = root ? uc_node_find(root, "name") : NULL;
    printf("YAML root=%s name=%s\n",
           root && root->kind == UC_NODE_MAP ? "map" : "?",
           name && name->kind == UC_NODE_SCALAR ? name->text : "?");
