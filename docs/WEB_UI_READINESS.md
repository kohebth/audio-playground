...
A view-independent Audio I/O control in the project topbar exposes persisted browser input/output selection in a
non-modal panel, so the transport remains usable while configuring or calibrating. AudioContext output routing is
...
The view-independent Audio I/O panel includes an opt-in five-second microphone latency profile. The AudioWorklet measures callback
...
Project readiness exposes the injected build commit, deployment base, and mode, and the explicitly public overdrive
