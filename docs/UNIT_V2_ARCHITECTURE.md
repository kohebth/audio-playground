...
`apg_v2_runtime_image_build(...)` creates an arena-owned runtime image descriptor from a compiled plan. It precomputes signal, param defaults, smoothing frames, meter, public audio port maps, bypass/project-mute metadata, per-node atom storage offsets, schedule view, state-buffer count/capacity/offsets, signal-array pointer-pool sizing, scalar refresh plans, copied signal-array indexes, and control-target layout metadata without allocating audio/runtime buffers. Scalar literal text is parsed by the compiler before runtime, and scalar refresh field/key validation stops at image build.
