...
`apg_v2_runtime_image_build(...)` creates an arena-owned runtime image descriptor from a compiled plan. It precomputes signal, param, meter, public audio port maps, bypass/project-mute metadata, per-node atom storage offsets, schedule, state-buffer count/capacity/offsets, signal-array pointer-pool sizing, scalar refresh plans, and control-target layout metadata without allocating audio/runtime buffers.
