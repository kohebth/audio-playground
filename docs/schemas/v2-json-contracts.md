...
`export --target wasm_realtime` currently returns `apg.project.export.v1` with `ok:true` and a generated scaffold bundle in `apg_project_wasm.json` plus `apg_project_wasm.mjs`. It remains a deterministic adapter shell, not a full AudioWorklet DSP executable yet.
