...
`export --target wasm_realtime` currently returns `apg.project.export.v2` with `ok:true` and a generated scaffold bundle in `apg_project_wasm.json`, `apg_project_wasm.mjs`, `apg_project_wasm_adapter.mjs`, and `apg_project_wasm_processor.js`. The manifest preserves the existing scaffold fields (`sample_rate`, `block_frames`, `runtime`, `layout`, and `status`) and adds an `artifacts` object naming the future `apg_project_wasm.wasm` module, adapter JS, entry JS, and AudioWorklet processor JS. The adapter exports `runtimeNames` for params, bypass instances, input meters, and output meters, validates those stable names for `setParam`, `setBypass`, `pollMeters`, and `getLastError`, and implements `compile`/`start`/`stop` by loading the generated AudioWorklet processor, creating an `AudioWorkletNode`, and disconnecting it. `wasm_module_available:false` means the bundle remains an AudioWorklet adapter shell, not a full WASM DSP executable yet.
