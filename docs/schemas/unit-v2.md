...
The v2 runtime supports mono processing through `apg_v2_runtime_process_mono(...)`, named host convenience ports through `apg_v2_runtime_process_mono_ports(...)`, and pre-resolved embedded ports through `apg_v2_runtime_process_mono_port_indices(...)`. Multi-channel audio ports use explicit per-channel signal mappings and can be processed with `apg_v2_runtime_process_interleaved_ports(...)` or the index-based `apg_v2_runtime_process_interleaved_port_indices(...)`. The runtime owns contiguous signal and parameter pools, param defaults, per-node atom call storage, and basic `FIELD_BUFFER` state allocation, then executes the compiled schedule. Control routing is param-only; non-param destinations are rejected by loader validation. Embedded callers should resolve names once during setup and use signal indices, port indices, and param/control indices in the real-time loop.
