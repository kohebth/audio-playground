...
Current repo CI provides host-side generated-bundle timing and generated-runner stack checks. Configure `APG_M7_C_COMPILER` to enable ARM/M7 syntax and stack-usage checks, `APG_M7_LINKER_SCRIPT` to link the generated bundle with a target memory map, and `APG_M7_BOARD_TIMING_COMMAND` to run a BSP timing command. CTest passes the generated export directory as the final argument through `benchmarks/apg_m7_board_timing_gate.sh`; the BSP command must print `m7_static_board_block_us=<value> budget_us=<value>`, and the gate fails missing, nonpositive, or over-budget measurements. Production board validation should configure `-DAPG_M7_REQUIRE_BOARD_TIMING=ON` so CMake fails immediately if no BSP timing command is supplied. STM32H7 production readiness still requires board/toolchain measurements.
