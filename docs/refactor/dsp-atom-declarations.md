...
`APG_ATOM_DEFINITIONS` is the declaration source of truth for all 71 atoms. The same list already drives registry rows
...
| API | Symbols |
|---|---:|
| Primary `atom_name_process(...)` | 71 |
| Additional spectral variants | 3 |
| Total declarations linked by the ABI test | 74 |
