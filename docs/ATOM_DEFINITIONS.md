...
the C ABI definition for the structures named by each row. Its compact type table has one row per atom with output, input,
parameter, and state field bodies. Family `*_field_descriptors.c` files remain the definitions of member offsets and buffer
defaults.
...
1. Add one ABI row to the type table in `dsp_types.h`, preserving the `out`, `in`, `params`, and `state` column order.
