...
int main(void) {
    ...
    return unit_host || project_host || runtime || !apg_process_context_valid(&context) ||
                   !apg_spectral_info_valid(&spectral) || !apg_stream_context_valid(&stream) ||
                   result.consumed_frames != 0u || result.produced_frames != 0u
               ? 1
               : 0;
}
