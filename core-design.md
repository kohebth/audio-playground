...
flowchart LR
    ...
    Registry["registry<br/>Register compact resources<br/>Contiguous buffers<br/>State blocks<br/>Param table<br/>Schedule table"]
...
---------------|-------------------------------------|-----------------------------|---------------------------------------------
 ...
 registry      | optimized graph                     | compact execution memory    | contiguous buffers, params, state, schedule
 runtime       | registry + audio/control input      | audio/control output        | real-time execution only
 measure       | runtime/registry state              | host-facing snapshots       | meters, diagnostics, readiness
...
-> registry builder: compact memory + schedule
-> runtime: execute only
-> measure: expose state to host/tooling

This direction also fits STM32H7 better because the runtime can eventually receive a prebuilt static registry instead of
allocating and resolving at startup.
