...
export function UnitSettingsDrawer({ file, open, unit, onChange, onClose, onReorderParam }: Props) {
  ...
  return (
    <>
      <button aria-label="Close Unit Settings" className="unit-settings-backdrop" onClick={onClose} type="button" />
      <aside
      ...
      >
        <header>
          <div><span>Atom Chain</span><strong>Unit Settings</strong></div>
          <button aria-label="Close Unit Settings" onClick={onClose} type="button">×</button>
        </header>
