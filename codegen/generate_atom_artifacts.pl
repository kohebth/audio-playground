...
sub generated_c_banner {
    my ($source) = @_;
    return "/* Generated from $source by codegen/generate_atom_artifacts.pl. Do not edit. */";
}
...
for my $family (@$families) {
    ...
    require_relative_path(
        $family->{header}, "$name header", qr{\Aapg-core/inc/atom/types/[a-z0-9_]+_types\.h\z}
    );
    require_relative_path(
        $family->{descriptor_source}, "$name descriptor_source",
        qr{\Aapg-core/src/atom/[a-z0-9_]+/[a-z0-9_]+_field_descriptors\.c\z}
    );
...
my %outputs;
$outputs{'apg-core/inc/atom/types/dsp_type_macros.h'} = render_type_macros_header();
for my $family (@$families) {
    $outputs{$family->{header}} = render_family_header($family);
    $outputs{$family->{descriptor_source}} = render_field_descriptors($family);
}
$outputs{'apg-core/inc/atom/generated/atom_definitions.generated.h'} = render_atom_definitions();
$outputs{'apg-core/inc/atom/generated/dsp_atoms.generated.h'} = render_dsp_declarations();
$outputs{'apg-core/src/apgcore/metadata/atom_catalog_contracts.generated.inc'} = render_catalog_contracts();
$outputs{'apg-web/src/atoms/atomCatalog.generated.ts'} = render_typescript_catalog();
