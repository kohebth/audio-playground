...
static int ctrl_find_runtime_step(runtime_unit_t *unit, const char *id) {
    ...
    for (int i = 0; i < unit->n_steps; i++) {
        if (unit->steps[i].id && strcmp(unit->steps[i].id, id) == 0)
            return i;
    }
    return -1;
}
...
static const atom_field_desc_t *ctrl_find_config_field(const atom_registry_entry_t *atom, const char *field_name) {
    ...
    for (int i = 0; i < atom->n_config_fields; i++) {
        if (strcmp(atom->config_fields[i].name, field_name) == 0)
...
static bool ctrl_add_binding(ctrl_param_t *param, runtime_unit_t *unit, int step_index, const char *field_name) {
    if (!param || !unit || step_index < 0 || step_index >= unit->n_steps || !field_name)
        return false;
    if (param->n_bindings >= CTRL_MAX_BINDINGS)
        return false;

    rt_step_t               *step  = &unit->steps[step_index];
    const atom_field_desc_t *field = ctrl_find_config_field(step->atom, field_name);
    if (!field || !step->config)
        return false;
    if (field->type != FIELD_FLOAT && field->type != FIELD_INT)
        return false;
...
static void ctrl_discover_bindings(ctrl_unit_t *ctrl, const uc_unit *uc) {
    ...
    for (size_t i = 0; i < uc->pipeline_len; i++) {
        ...
        for (size_t j = 0; j < stage->config_len; j++) {
            char param_name[64];
            if (!ctrl_parse_param_ref(stage->config[j].value.text, param_name, sizeof(param_name)))
                continue;
...
bool ctrl_unit_process_frames(ctrl_unit_t *ctrl, float *in, float *out, uint32_t frames) {
    if (!ctrl || !ctrl->unit || frames == 0u)
        return false;
    ctrl_unit_tick(ctrl, (int)frames);
    return runtime_unit_process_frames(ctrl->unit, in, out, frames);
}

void ctrl_unit_process(ctrl_unit_t *ctrl, float *in, float *out) {
    if (!ctrl || !ctrl->unit)
        return;
    (void)ctrl_unit_process_frames(ctrl, in, out, (uint32_t)ctrl->chunk_length);
}
