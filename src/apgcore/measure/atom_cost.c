...
bool apg_atom_estimate_cost(
    ...
    if (!entry || !entry->name || !out)
        return false;

    memset(out, 0, sizeof(*out));
    const uint64_t frames = apg_process_frames_or_default(process_info);
    ...
    if (atom_is(entry, "generation_dc"))
        ...
        const uint32_t taps = params ? clamp_items_i32(params->kernel_size, APG_DEFAULT_FIR_TAPS, 1024u)
                                     : APG_DEFAULT_FIR_TAPS;
        ...
        const uint32_t taps = params ? clamp_items_i32(params->num_taps, APG_DEFAULT_SINC_TAPS, 63u)
                                     : APG_DEFAULT_SINC_TAPS;
        ...
        cpu = sat_add_u64(linear_cost(24u, frames, sat_mul_u64(num_out, 2u)),
...
bool apg_graph_estimate_cost(
    ...
    for (size_t i = 0u; i < count; ++i) {
        ...
        if (!apg_atom_estimate_cost(entries[i], config, process_info, spectral, &atom_cost))
            return false;
