...
static int             accept_(ps_t *P, uc_tok_kind k) {
    if (pk(P, 0)->kind == k) {
        ++P->p;
        return 1;
    }
...
static void skip_blank(ps_t *P) {
    for (;;) {
        if (pk(P, 0)->kind == UC_TOK_INDENT && pk(P, 1)->kind == UC_TOK_NEWLINE) {
            P->p += 2;
            continue;
        }
        if (pk(P, 0)->kind == UC_TOK_NEWLINE) {
            ++P->p;
            continue;
        }
        break;
...
static int map_push(uc_arena *a, uc_node *m, const char *key, uc_node *v) {
    if (m->map_len == m->map_cap) {
        ...
        if (!nb)
            return -1;
        if (m->map)
            memcpy(nb, m->map, m->map_len * sizeof *nb);
        m->map     = nb;
        m->map_cap = nc;
    }
...
static int seq_push(uc_arena *a, uc_node *s, uc_node *v) {
    if (s->seq_len == s->seq_cap) {
        ...
        if (!nb)
            return -1;
        if (s->seq)
            memcpy(nb, s->seq, s->seq_len * sizeof *nb);
        s->seq     = nb;
        s->seq_cap = nc;
    }
    s->seq[s->seq_len++] = v;
...
static uc_node *parse_flow_sequence(ps_t *P);
...
static uc_node *parse_flow_value(ps_t *P) {
    const uc_token *token = pk(P, 0);
    uc_node        *value = NULL;
    switch (token->kind) {
    case UC_TOK_VARREF:
        value = node_new(P->arena, UC_NODE_VARREF);
        if (value) {
            value->text     = token->text;
            value->text_len = token->len;
            ++P->p;
        }
        break;
    case UC_TOK_NULL:
        value = node_new(P->arena, UC_NODE_NULL);
        ++P->p;
        break;
    case UC_TOK_LBRACE:
        value = parse_flow_map(P);
        break;
    case UC_TOK_LBRACKET:
        value = parse_flow_sequence(P);
        break;
    case UC_TOK_SCALAR:
        value = node_new(P->arena, UC_NODE_SCALAR);
        if (value) {
            value->text     = token->text;
            value->text_len = token->len;
            ++P->p;
        ...
        break;
    default:
        uc_error_set(P->err, UC_E_PARSE, token->loc, "invalid flow value");
        return NULL;
    }
    return value;
}
...
static uc_node *parse_flow_map(ps_t *P) {
    ...
    if (pk(P, 0)->kind != UC_TOK_RBRACE) {
        for (;;) {
            ...
            ++P->p;
            if (expect(P, UC_TOK_COLON, "':'") < 0)
                return NULL;

            const uc_token *vt = pk(P, 0);
            uc_node        *v  = parse_flow_value(P);
            if (!v)
                return NULL;
            ...
            if (accept_(P, UC_TOK_COMMA))
                continue;
            break;
        }
    }
    if (expect(P, UC_TOK_RBRACE, "'}'") < 0)
        return NULL;
    accept_(P, UC_TOK_NEWLINE);
    return m;
}
...
static uc_node *parse_flow_sequence(ps_t *P) {
    if (expect(P, UC_TOK_LBRACKET, "'['") < 0)
        return NULL;
    uc_node *sequence = node_new(P->arena, UC_NODE_SEQ);
    if (!sequence)
        return NULL;

    if (pk(P, 0)->kind != UC_TOK_RBRACKET) {
        for (;;) {
            const uc_token *token = pk(P, 0);
            uc_node        *value = parse_flow_value(P);
            if (!value)
                return NULL;
            if (seq_push(P->arena, sequence, value) < 0) {
                uc_error_set(P->err, UC_E_OOM, token->loc, "arena OOM");
                return NULL;
            }
            if (accept_(P, UC_TOK_COMMA))
                continue;
...
static uc_node *parse_mapping(ps_t *P, int indent) {
    ...
    for (;;) {
        skip_blank(P);
        if (pk(P, 0)->kind != UC_TOK_INDENT || pk(P, 0)->indent != indent)
            break;
        if (pk(P, 1)->kind != UC_TOK_SCALAR)
            break;
        ++P->p;
        const uc_token *kt = eat(P);
        if (expect(P, UC_TOK_COLON, "':' after key") < 0)
            return NULL;
    ...
    for (;;) {
        ...
        if (pk(P, 0)->kind == UC_TOK_SCALAR && pk(P, 1)->kind == UC_TOK_COLON) {
            ...
            if (!m)
                return NULL;
            const uc_token *kt = eat(P);
            if (expect(P, UC_TOK_COLON, "':' after list-item key") < 0)
                return NULL;
            ...
            for (;;) {
                skip_blank(P);
                if (pk(P, 0)->kind != UC_TOK_INDENT || pk(P, 0)->indent != child_indent)
                    break;
                if (pk(P, 1)->kind != UC_TOK_SCALAR)
                    break;
                ++P->p;
                ...
                if (!v2)
                    return NULL;
                if (map_push(P->arena, m, kt2->text, v2) < 0)
                    return NULL;
            }
            if (seq_push(P->arena, s, m) < 0)
                return NULL;
        } else {
            uc_node *v = parse_value(P, child_indent);
            if (!v)
                return NULL;
            if (seq_push(P->arena, s, v) < 0)
                return NULL;
        }
    }
    return s;
...
static uc_node *parse_block(ps_t *P, int min_indent) {
    skip_blank(P);
    if (pk(P, 0)->kind != UC_TOK_INDENT)
        return NULL;
    if (pk(P, 0)->indent < min_indent)
        return NULL;
    int indent = pk(P, 0)->indent;
    return (pk(P, 1)->kind == UC_TOK_DASH) ? parse_sequence(P, indent) : parse_mapping(P, indent);
}

uc_status uc_parse(const uc_token_vec *tokens, uc_arena *arena, uc_node **out_root, uc_error *err) {
    ps_t P  = {0};
    P.t     = tokens->data;
    P.n     = tokens->len;
    P.arena = arena;
